...
至此简单入门的`docker`教程就全部结束了，我们首先安装docker -> 创建容器 -> 搜索拉取镜像 -> 构建自己的镜像(`Dockerfile`) -> 发布镜像，文章大部分其实参阅了各个平台的基础或者文档。简单的docker就这样跑起来了，但是真正我们去使用的时候往往会有很多问题，用法也不是直接这么暴力的去使用，例如直接拉取`centos`镜像往里面安装`nginx`，会很浪费资源，也没有做到真正的隔离环境。下一篇讲使用`docker-compose`讲多个容器组合起来，尽可能实战操作
